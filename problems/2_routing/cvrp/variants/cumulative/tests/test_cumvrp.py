...
_inst = _load_mod("cumvrp_inst_test", os.path.join(_variant_dir, "instance.py"))
CumVRPInstance = _inst.CumVRPInstance
CumVRPSolution = _inst.CumVRPSolution
validate_solution = _inst.validate_solution
small_cumvrp_6 = _inst.small_cumvrp_6

_heur = _load_mod("cumvrp_heur_test", os.path.join(_variant_dir, "heuristics.py"))
nearest_neighbor_cum = _heur.nearest_neighbor_cum

_meta = _load_mod("cumvrp_meta_test", os.path.join(_variant_dir, "metaheuristics.py"))
simulated_annealing = _meta.simulated_annealing


class TestCumVRPInstance:
    def test_random(self):
        inst = CumVRPInstance.random(n=8, seed=42)
        assert inst.n == 8
...
class TestCumVRPSA:
    ...
    def test_competitive(self):
        inst = CumVRPInstance.random(n=8, seed=42)
        nn = nearest_neighbor_cum(inst)
        sa = simulated_annealing(inst, max_iterations=10000, seed=42)
        assert sa.total_latency <= nn.total_latency + 1e-6

    def test_determinism(self):
        inst = CumVRPInstance.random(n=6, seed=42)
        s1 = simulated_annealing(inst, max_iterations=3000, seed=42)
        s2 = simulated_annealing(inst, max_iterations=3000, seed=42)
        assert abs(s1.total_latency - s2.total_latency) < 1e-6

    def test_time_limit(self):
        inst = CumVRPInstance.random(n=10, seed=42)
